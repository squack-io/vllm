...
class NGramMatchStats:
    """Statistics for ngram matches and rejections."""
    def __init__(self):
        self.total_attempts = 0
        self.matches_by_window = {}  # window_size -> num_matches
        self.no_match_count = 0
        
    def record_match(self, window_size: int):
        """Record a successful match at given window size."""
        self.total_attempts += 1
        self.matches_by_window[window_size] = self.matches_by_window.get(window_size, 0) + 1
    ...
    def get_stats(self):
        ...
        stats = {
            ...
            'total_matches': sum(self.matches_by_window.values()),
            'no_matches': self.no_match_count,
            'matches_by_window': dict(sorted(self.matches_by_window.items()))
        }
        if self.total_attempts > 0:
            stats['match_rate'] = (stats['total_matches'] / self.total_attempts) * 100
        return stats
        ...
        for idx, seq_group_metadata in enumerate(
            ...
            for ngram_size in range(
                ...
                if first_match.values.item():
                    match_found = True
                    self.match_stats.record_match(ngram_size)
                    ...
                    break
            if not match_found:
                self.match_stats.record_no_match()
