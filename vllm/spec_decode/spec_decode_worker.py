...
from vllm.spec_decode.test_worker import TestWorker
...
def create_spec_worker(*args, **kwargs) -> "SpecDecodeWorker":
    ...
    spec_decode_worker = SpecDecodeWorker.create_worker(
        ...
        disable_by_batch_size=speculative_config.speculative_disable_by_batch_size,
        draft_token_acceptance_method=speculative_config.draft_token_acceptance_method,
        typical_acceptance_sampler_posterior_threshold=speculative_config.typical_acceptance_sampler_posterior_threshold,
        typical_acceptance_sampler_posterior_alpha=speculative_config.typical_acceptance_sampler_posterior_alpha,
...
class SpecDecodeWorker(LoraNotSupportedWorkerBase):
    ...
    * The scoring forward pass is done without an MQA kernel, which is
    ...
    def create_worker(
        ...
        ngram_prompt_lookup_max = draft_worker_kwargs.pop("ngram_prompt_lookup_max")
        ngram_prompt_lookup_min = draft_worker_kwargs.pop("ngram_prompt_lookup_min")
            ...
            if draft_model_config.hf_config.model_type == "mlp_speculator":
                ...
                if draft_tp == 1:
                    ...
                    if draft_model_config.hf_config.model_type == "eagle":
                        raise NotImplementedError("EAGLE does not support TP > 1 yet")
        ...
        logger.info(
            "Configuring SpecDecodeWorker with proposer=%s", type(proposer_worker)
        )
        ...
        if draft_token_acceptance_method == "rejection_sampler":
            ...
            spec_decode_sampler = TypicalAcceptanceSampler(
                posterior_threshold=typical_acceptance_sampler_posterior_threshold,
                posterior_alpha=typical_acceptance_sampler_posterior_alpha,
            )
        logger.info(
            "[Speculative Decoding] Configuring" " SpecDecodeWorker with sampler=%s",
        ...
        if not disable_mqa_scorer:
            ...
            if (
                draft_model_config
                and draft_model_config.max_model_len
    ...
    def __init__(
        ...
        self._metrics = (
            AsyncMetricsCollector(self.spec_decode_sampler)
    ...
    def init_device(self) -> None:
        ...
        if self.disable_mqa_scorer:
            ...
            logger.info(
                "[Speculative Decoding] Use batch " "expansion for scoring proposals."
            )
        else:
    ...
    def _configure_model_sampler_for_spec_decode(self):
        ...
        (
            self.scorer_worker.model_runner.model.sampler.should_modify_greedy_probs_inplace
        ) = True
    ...
    def initialize_cache(self, num_gpu_blocks: int, num_cpu_blocks: int) -> None:
        ...
        self.scorer_worker.initialize_cache(
    ...
    def execute_model(
        ...
        if no_spec:
            return self._run_no_spec(
                execute_model_req, skip_proposer=disable_all_speculation
            )
    ...
    def _serialize_sampler_output_no_logprobs(
        self, execute_model_req: ExecuteModelRequest, sampler_output: SamplerOutput
    ) -> SamplerOutput:
        """
        ...
        seq_output_prompt_logprobs = [
            seq.is_prompt
            and seq.sampling_params.prompt_logprobs is not None
        ...
        sampled_token_ids_list = (
            ...
            if any(seq_output_prompt_logprobs)
        ...
        completion_seq_group_output_list: List[CompletionSequenceGroupOutput] = []
        for index, ((seq_id, seq_data), needs_prompt_logprobs) in enumerate(
            zip(seq_data_entries, seq_output_prompt_logprobs)
        ):
    ...
    def _verify_tokens(
        ...
        if hidden_states is not None:
            ...
            self.previous_hidden_states = HiddenStates(
                hidden_states, seq_group_metadata_list, second_last_token_hidden_states
            )
    ...
    def _create_output_sampler_list(
        ...
        if self._disable_logprobs:
            ...
            (
                ...
                batch_size, num_steps, self.scorer_worker.model_config.max_logprobs
            )
        else:
    ...
    def _create_dummy_logprob_lists(
        ...
        """
        accepted_token_id_ranks_by_step = [[-1] * batch_size for _ in range(num_steps)]
        accepted_token_id_logprobs_by_step = [
            [0.0] * batch_size for _ in range(num_steps)
        ]
        topk_logprobs_by_step: List[List[List[Optional[float]]]] = [
            [[None] * num_top_k for _ in range(batch_size)] for _ in range(num_steps)
        ]
    ...
    def _create_logprob_lists_from_tensors(
        ...
        (topk_logprobs_by_step_tensor, topk_indices_by_step_tensor) = (
            target_logprobs_by_step.topk(
                k=num_top_k,
                dim=-1,
            )
        )
...
def prepare_prefill_hidden_states(prefill_hidden_states: torch.Tensor) -> HiddenStates:
    ...
    return (
        HiddenStates(prefill_hidden_states.roll(shifts=1, dims=0))
